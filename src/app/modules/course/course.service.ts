...
const insertIntoDB = async (data: ICourseCreateData): Promise<any> => {
  ...
  const newCourse = await prisma.$transaction(async tranasactionClient => {
    const result = await tranasactionClient.course.create({
      data: courseData,
    });
    ...
    //   for (let index = 0; index < preRequisiteCourses.length; index++) {
    //     const createPrerequisite =
    //       await tranasactionClient.courseToPrerequisite.create({
    //         data: {
    //           courseId: result.id,
    //           preRequisiteId: preRequisiteCourses[index].courseId,
    //         },
...
const updateOneInDB = async (
  ...
  await prisma.$transaction(async trnasactionClient => {
    const result = await trnasactionClient.course.update({
      where: {
        id,
      },
      data: courseData,
    ...
    if (preRequisiteCourses && preRequisiteCourses.length > 0) {
      ...
      //   await trnasactionClient.courseToPrerequisite.deleteMany({
      //     where: {
      //       AND: [
      //         {
      //           courseId: id,
      ...
      //   await trnasactionClient.courseToPrerequisite.create({
      //     data: {
      //       courseId: id,
